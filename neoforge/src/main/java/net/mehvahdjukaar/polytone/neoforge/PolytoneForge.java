...
public class PolytoneForge {
    ...
    public PolytoneForge(IEventBus modBus) {
        ...
        if (FMLEnvironment.dist == Dist.CLIENT) {
            ...
            modBus.addListener(this::addAutoGeneratedModels);
        } else {
    ...
    public void addAutoGeneratedModels(ModelEvent.ModifyBakingResult event) {
        Function<Material, TextureAtlasSprite> spriteGetter = event.getTextureGetter();
        ModelBakery modelBakery = event.getModelBakery();
        for (var model : Polytone.ITEM_MODELS.getAutoGeneratedModels()) {
            String texturePath = model.texturePath().toString();
            BlockModel baseModel = BlockModel.fromString("{\n" +
            ...
            ModelResourceLocation modelRes = ModelResourceLocation.standalone(model.modelPath());
     /*

            ModelBakery.ModelBakerImpl baker = modelBakery.new ModelBakerImpl(null, modelRes);
            BakedModel baked = ITEM_MODEL_GENERATOR.generateBlockModel(spriteGetter, baseModel)
                    .bake(null, baseModel, spriteGetter, BlockModelRotation.X0_Y0, false);
