 ...
 * reloaders fill textures with the same naming primitives ({@link TintedTextures}) and the
 * editor renders {@link #expectedSlots} directly, so the two can't drift apart.
 *
 * <p>Pure logic over simple file names (with extension) and the JSON's {@code stem} (its file
 * name without extension): no IO and no Minecraft types, so both the {@code Identifier}-keyed
