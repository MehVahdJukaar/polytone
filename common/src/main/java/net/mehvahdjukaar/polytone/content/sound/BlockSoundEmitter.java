...
public record BlockSoundEmitter(
    ...
    public static final Codec<BlockSoundEmitter> CODEC = RecordCodecBuilder.create(i -> i.group(
            ...
            IBlockExp.CODEC_LEGACY.optionalFieldOf("chance", IBlockExp.ONE).forGetter(BlockSoundEmitter::chance),
            IBlockExp.CODEC_LEGACY.optionalFieldOf("x", IBlockExp.ZERO).forGetter(BlockSoundEmitter::x),
            IBlockExp.CODEC_LEGACY.optionalFieldOf("y", IBlockExp.ZERO).forGetter(BlockSoundEmitter::y),
            IBlockExp.CODEC_LEGACY.optionalFieldOf("z", IBlockExp.ZERO).forGetter(BlockSoundEmitter::z),
            IBlockExp.CODEC_LEGACY.optionalFieldOf("volume", IBlockExp.ZERO).forGetter(BlockSoundEmitter::volume),
            IBlockExp.CODEC_LEGACY.optionalFieldOf("pitch", IBlockExp.ZERO).forGetter(BlockSoundEmitter::pitch),
    ...
    public void tick(Level level, BlockPos pos, BlockState state, TickSource source) {
        ...
        double spawnChance = chance.evaluate(level, center, state);
        if (level.random.nextFloat() < spawnChance) {
            ...
            Vec3 vec = center.add(
                    x.evaluate(level, center, state),
                    y.evaluate(level, center, state),
                    z.evaluate(level, center, state));

            float v = (float) volume.evaluate(level, center, state);
            float p = (float) pitch.evaluate(level, center, state);
