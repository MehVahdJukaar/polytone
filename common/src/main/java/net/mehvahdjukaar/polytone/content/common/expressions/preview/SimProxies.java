...
/**
 * The full set of sim proxies for one editor preview. Installed via {@link PreviewContext}; the MVEL
 * variable binding swaps each one in for its live singleton (see ExpUtils). Each preview panel owns
 * its own instance, so slider state is independent per open tab.
 */
public final class SimProxies {
    ...
    /** All inputs across proxies, in UI display order. */
