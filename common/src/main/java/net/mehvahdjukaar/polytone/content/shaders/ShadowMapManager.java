...
public class ShadowMapManager extends SingleJsonOrPropertiesReloadListener {
    ...
    // Internals, overridable per-pack via polytone/shadow_map.json (defaults in ShadowMapSettings).
    private int shadowRes = ShadowMapSettings.DEFAULT.resolution();     // texture size, px (square)
    private float coverage = ShadowMapSettings.DEFAULT.coverage();      // ortho half-width, blocks
    private float depthRange = ShadowMapSettings.DEFAULT.depthRange();  // ortho half-depth, blocks
    private float updateInterval = ShadowMapSettings.DEFAULT.updateInterval();  // min ticks between renders
    ...
    // ----- single-file settings reload (polytone/shadow_map.json, decoded by ShadowMapSettings.CODEC) -----
    ...
    protected void parseWithLevel(Map<ResourceLocation, JsonElement> jsons, RegistryOps<JsonElement> ops, RegistryAccess access) {
        ...
        for (var entry : jsons.entrySet()) { // last (highest priority) wins; a missing file keeps defaults
            try {
                result = ShadowMapSettings.CODEC.parse(ops, entry.getValue()).getOrThrow();
            } catch (Exception e) {
    ...
    protected void applyWithLevel(RegistryAccess access, boolean isLogIn) {
        applyInternals(parsedSettings);
    }

    @Override
    protected void resetWithLevel(boolean logOff) {
        this.parsedSettings = ShadowMapSettings.DEFAULT;
        applyInternals(ShadowMapSettings.DEFAULT);
    }

    private void applyInternals(ShadowMapSettings s) {
        this.coverage = s.coverage();
        this.depthRange = s.depthRange();
        this.shadowRes = s.resolution();       // target recreated lazily by ensureTarget()
        this.updateInterval = s.updateInterval();
        this.hasRendered = false;              // force a fresh render on the next frame after a reload
    }
