...
// "type" dispatch (custom, semi_custom, gpu) with the pre-"type" reading as fallback: no "type" and a
// "copy_from" means semi custom, no "type" and anything else means custom. The fallback refuses
// files that do have a "type", so a broken gpu file fails as gpu instead of silently decoding as
// custom, which ignores unknown keys.
