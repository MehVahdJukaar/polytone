...
public class BlockOffsets {
    ...
    public static final Codec<BlockBehaviour.OffsetFunction> CODEC = CodecUtil.withAlternative(
    ...
    private record TypeOffset(BlockBehaviour.OffsetType type,
        ...
        private TypeOffset(BlockBehaviour.OffsetType type) {
            this(type, BlockBehaviour.Properties.of().offsetType(type)
                    .offsetFunction.orElse((a, b, c) -> Vec3.ZERO));
        }
    ...
    private record CustomOffset(float maxX, float maxY, float maxZ) implements BlockBehaviour.OffsetFunction {
        ...
        @Override
        public Vec3 evaluate(BlockState blockState, BlockGetter getter, BlockPos blockPos) {
