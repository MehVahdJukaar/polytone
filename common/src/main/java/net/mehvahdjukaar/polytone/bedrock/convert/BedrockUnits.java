...
/**
 * Bedrock states everything in seconds, meters and degrees and evaluates per frame; we tick 20 times a
 * second and rotate in radians. Every conversion goes through here so the factors live in one place.
 */
public class BedrockUnits {
    ...
    /**
     * Bedrock drag is a coefficient applied per second as {@code v -= v * drag * dt}. Per tick that
     * leaves {@code v * (1 - drag/20)}, which is the multiplier our ticker needs. Clamped at 0 because
     * a drag above 20 would otherwise flip the velocity sign every tick instead of stopping it.
     */
