...
final class BiomeSceneRenderPass {
    ...
    static void render(SceneCamera camera, int width, int height, Colors colors,
        ...
        // 26.2 has no immediate-mode buffer source: geometry is submitted into a node storage and
        // drawn by the feature dispatcher. The storage is local, so nothing leaks into the game's frame.
        SubmitNodeStorage submitNodes = new SubmitNodeStorage();

        drawBackdropAndSky(pose, submitNodes, colors, ex, ey, ez);
        drawWater(pose, submitNodes, colors, water);
        mc.gameRenderer.featureRenderDispatcher().renderAllFeatures(submitNodes);

        drawBlocks(mc, pose, colors, blocks);
    }

    private static void drawBackdropAndSky(PoseStack pose, SubmitNodeStorage submitNodes, Colors colors,
                                           float ex, float ey, float ez) {
        float[] sky = rgb(colors.sky());
        float[] fog = rgb(colors.fog());
        submitNodes.submitCustomGeometry(pose, RenderTypes.debugQuads(), (p, c) -> {
    ...
    private static void drawWater(PoseStack pose, SubmitNodeStorage submitNodes, Colors colors, WaterQuad w) {
