...
public final class ParticlePreview extends ExpressionPreview {
    ...
    private final class ParticleScene implements LiveViewport.Renderer {
        ...
        public void render(SceneCamera camera, int width, int height) {
            ...
            // Our own storage (26.2 passes it into renderAllFeatures): the preview submits outside the
            // game's own frame, so nothing it queues may end up in the level pass.
            ...
            try {
                particlesRenderState.submit(submitNodes, camState);
        ...
        // per-vertex LineWidth element this path can't supply. Camera view baked into the pose
        ...
        private void drawReference(SceneCamera camera) {
            ...
            SubmitNodeStorage submitNodes = new SubmitNodeStorage();
            submitNodes.submitCustomGeometry(pose, RenderTypes.debugQuads(), (ps, c) -> {
